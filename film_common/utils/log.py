...
def set_verbosity_level(lvl):
    """lvl: 0 means minimal (warnings), 1 normal (and info), 2 maximal
       (and debug).
    """
    for h in itertools.chain(logging.getLogger('film20').handlers,
                             logging.getLogger().handlers):
        if isinstance(h, logging.StreamHandler):
            h.level = VERBOSITY_LEVELS[lvl]
