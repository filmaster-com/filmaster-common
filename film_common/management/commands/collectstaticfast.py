...
def process_tasks():
    for obj, method_name, args, kwargs in iter(task_queue.get, None):
        getattr(obj, method_name)(*args, **kwargs)
        task_queue.task_done()
    task_queue.task_done() # for last None item
...
class Command(NoArgsCommand):
    ...
    option_list = NoArgsCommand.option_list + (
        ...
        make_option('-p', '--pattern', action='append', default=[],
        ...
        make_option('--no-default-ignore', action='store_false',
            ...
            help="Don't ignore the common private glob-style patterns 'CVS', "
                "'.*' and '*~'."),
        make_option('--concurency', '-c', type=int, default=1),
    )
    ...
    def handle_noargs(self, **options):
        ...
        for i in range(concurency):
            Process(target=process_tasks).start()
        ...
        for finder in finders.get_finders():
            for path, storage in finder.list(ignore_patterns):
                ...
                if getattr(storage, 'prefix', None):
                    ...
                    prefixed_path = path
                if symlink:
                    task_queue.put((self.copier, 'link_file', (path, prefixed_path, storage), options))
                else:
                    task_queue.put((self.copier, 'copy_file', (path, prefixed_path, storage), options))
